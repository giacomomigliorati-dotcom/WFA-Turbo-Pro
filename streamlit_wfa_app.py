...
h1 {
  ...
  background-clip: text; padding-bottom: 4px;
}
h2 { color: var(--oz-cyan) !important; font-weight: 600 !important;
  border-bottom: 1px solid var(--oz-border); padding-bottom: 6px; margin-bottom: 12px; }
h3 { color: #7dd3fc !important; font-weight: 500 !important; }
[data-testid="stSidebar"], section[data-testid="stSidebar"] > div {
  background-color: #101820 !important; border-right: 1px solid var(--oz-border) !important; }
[data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3 {
  color: var(--oz-cyan) !important; font-size: 0.85rem !important;
  text-transform: uppercase; letter-spacing: 0.08em; }
[data-testid="stMetric"] {
  background: var(--oz-panel) !important; border: 1px solid var(--oz-border) !important;
  border-radius: 8px !important; padding: 14px 16px !important; }
[data-testid="stMetricLabel"] { color: var(--oz-muted) !important; font-size: 0.72rem !important;
...
.stButton > button, [data-testid="baseButton-secondary"] {
  ...
  font-family: 'Inter', sans-serif !important; letter-spacing: 0.03em; transition: all 0.15s ease; }
...
/* Pulsante Lancia WFA – evidenziato */
...
[data-testid="stDownloadButton"] > button {
  ...
  border: 1px solid var(--oz-blue) !important; color: #93c5fd !important;
  border-radius: 6px !important; font-weight: 500 !important; }
[data-testid="stDownloadButton"] > button:hover { box-shadow: 0 0 10px rgba(59,130,246,0.3) !important; }
[data-testid="stFileUploader"] { border: 1px dashed var(--oz-border) !important;
  border-radius: 8px !important; background: var(--oz-panel) !important; padding: 6px; }
[data-testid="stExpander"] { border: 1px solid var(--oz-border) !important;
  border-radius: 8px !important; background: var(--oz-panel) !important; }
...
[data-testid="stTabs"] [data-baseweb="tab"][aria-selected="true"] {
  color: var(--oz-cyan) !important; border-bottom: 2px solid var(--oz-cyan) !important; }
...
**3. Omega Ratio (ranking IS)** — Rapporto somma rendimenti positivi / |negativi| su finestra 12 mesi.

**4. CUSUM Weekday Killer** — Se una combinazione strategia/giorno accumula perdite anomale (CUSUM > soglia),
quel giorno viene messo in Blacklist per la finestra OOS successiva.

**5. Anti-overlap + rotazione frazionata** — Le migliori N strategie (Top-N configurabile) vengono selezionate
rispettando il limite massimo per gruppo. I rank dal 1 al "soglia pieno" ricevono peso pieno;
i restanti ricevono il peso ridotto (entrambi configurabili).

**6. OOS filtrato** — I trade nella finestra OOS di 28 giorni vengono filtrati dalla blacklist CUSUM
e moltiplicati per il peso assegnato.

---

### 🚀 Avvio simulazione
Dopo aver caricato il CSV e configurato i gruppi nella sidebar, premi **▶ Lancia simulazione WFA**
sulla pagina principale. Il calcolo **non parte automaticamente** per evitare rallentamenti.

---

### 💾 Backup configurazione
1. Configura gruppi e parametri nella sidebar
2. **📤 Esporta configurazione JSON** → salva `texano_config.json`
3. Al prossimo accesso: **📥 Importa** → **✅ Applica** → tutto ripristinato
        """)
...
DEFAULT_TOP_N = 7
DEFAULT_FULL_WEIGHT_COUNT = 5   # rank 1-5 → peso pieno
DEFAULT_FULL_WEIGHT_PCT = 100   # % peso pieno
DEFAULT_BENCH_WEIGHT_PCT = 50   # % peso panchina
...
def apply_config_payload(payload, all_strategies):
    ...
    st.session_state["max_per_group"] = max(1, min(DEFAULT_TOP_N, int(max_per_group)))
    top_n = payload.get("top_n", DEFAULT_TOP_N)
    st.session_state["top_n"] = max(1, int(top_n))
    fwc = payload.get("full_weight_count", DEFAULT_FULL_WEIGHT_COUNT)
    st.session_state["full_weight_count"] = max(0, int(fwc))
    fwp = payload.get("full_weight_pct", DEFAULT_FULL_WEIGHT_PCT)
    st.session_state["full_weight_pct"] = max(1, min(100, int(fwp)))
    bwp = payload.get("bench_weight_pct", DEFAULT_BENCH_WEIGHT_PCT)
    st.session_state["bench_weight_pct"] = max(1, min(100, int(bwp)))

# ─── INIT SESSION STATE PARAMETRI WFA ────────────────────────────────────────
if "top_n" not in st.session_state:
    st.session_state["top_n"] = DEFAULT_TOP_N
if "full_weight_count" not in st.session_state:
    st.session_state["full_weight_count"] = DEFAULT_FULL_WEIGHT_COUNT
if "full_weight_pct" not in st.session_state:
    st.session_state["full_weight_pct"] = DEFAULT_FULL_WEIGHT_PCT
if "bench_weight_pct" not in st.session_state:
    st.session_state["bench_weight_pct"] = DEFAULT_BENCH_WEIGHT_PCT
...
if uploaded_file is not None:
    ...
    st.sidebar.caption("Esporta per salvare gruppi e parametri. Reimporta dopo ogni refresh.")
    ...
    top_n = st.sidebar.number_input(
        "Numero massimo strategie selezionate (Top-N)",
        min_value=1, max_value=20,
        value=st.session_state["top_n"], step=1,
        help="Quante strategie entrano nel portafoglio per ogni finestra OOS. Default: 7"
    )
    st.session_state["top_n"] = int(top_n)

    full_weight_count = st.sidebar.number_input(
        ...
        help="Le prime N strategie in classifica ricevono peso pieno. Le restanti ricevono peso ridotto."
    )
    st.session_state["full_weight_count"] = int(full_weight_count)

    full_weight_pct = st.sidebar.number_input(
        "Peso pieno (%)",
        min_value=1, max_value=100,
        value=st.session_state["full_weight_pct"], step=5,
        help="Percentuale applicata alle strategie 'core' (rank ≤ soglia). Default: 100%"
    )
    st.session_state["full_weight_pct"] = int(full_weight_pct)

    bench_weight_pct = st.sidebar.number_input(
        "Peso panchina (%)",
        min_value=1, max_value=100,
        value=st.session_state["bench_weight_pct"], step=5,
        help="Percentuale applicata alle strategie 'panchina' (rank > soglia). Default: 50%"
    )
    st.session_state["bench_weight_pct"] = int(bench_weight_pct)

    # Anteprima schema pesi
    bench_count = int(top_n) - int(full_weight_count)
    st.sidebar.markdown(
        f"**Schema attuale:** {int(full_weight_count)} × {int(full_weight_pct)}% "
        f"+ {bench_count} × {int(bench_weight_pct)}%"
    )
    ...
    max_per_group = st.sidebar.number_input(
        "Max strategie dello stesso gruppo nel portafoglio",
        min_value=1, max_value=int(top_n),
        value=min(st.session_state['max_per_group'], int(top_n)),
    ...
    if st.session_state['group_names']:
        group_to_rename = st.sidebar.selectbox("Seleziona gruppo da rinominare", options=st.session_state['group_names'], key="group_to_rename")
    ...
    with st.sidebar.expander("📋 Modifica gruppo di ogni strategia", expanded=True):
        ...
        if st.button("✅ Salva assegnazioni"):
            ...
            st.sidebar.success("Assegnazioni aggiornate — esporta il JSON per salvarle!")
            st.rerun()
    ...
    with col_info:
        st.markdown(
            ...
            f"Max/gruppo: <b style='color:#00d4ff'>{int(max_per_group)}</b>"
            f"</div>",
            unsafe_allow_html=True
        )

    if launch:
        st.session_state["wfa_results"] = None  # reset risultati precedenti

    # Legge i parametri attivi al momento del click e li salva assieme ai risultati
    if launch:
    ...
    # ─── RISULTATI (mostrati se esistono in session_state) ───────────────────
    if st.session_state.get("wfa_results") is not None:
        ...
        val_col1.info(f"📅 **Dal:** {latest_start.strftime('%d %b %Y')}")
        val_col2.info(f"🔚 **Fino al:** {latest_end.strftime('%d %b %Y')}")
        if days_left > 0:
            val_col3.warning(f"⏳ **Rotazione tra:** {days_left}gg")
        else:
            val_col3.error("🔄 **Rotazione scaduta**")
        val_col4.info(f"🎛️ Top-N: **{run_params['top_n']}** | Max/gr: **{run_params['max_per_group']}**")
        val_col5.info(
            f"⚖️ Pesi: **{run_params['full_weight_count']}×{run_params['full_weight_pct']}%** "
        ...
        if not exclusions_df.empty:
            ...
            if not latest_excl.empty:
                st.warning("⚠️ Strategie escluse — CUSUM ha bannato tutti i giorni operativi:")
