...
if uploaded_file is not None:
    with st.spinner('Elaborazione in corso...'):
        ...
        df_filtered['P/L %'] = df_filtered['P/L %'].apply(lambda x: float(str(x).replace('%', '').replace(',', '')) if pd.notna(x) else np.nan)
        ...
        df_filtered['Net PnL'] = df_filtered['P/L'] - df_filtered['Opening Commissions + Fees'] - df_filtered['Closing Commissions + Fees']
        ...
        while current_oos_start <= max_date:
            ...
            for strat in valid_strats:
                ...
                banned_days = []
                for day in range(7):
                    day_data = strat_is[strat_is['weekday_open'] == day].sort_values(by='Date Closed')
                    if len(day_data) >= 10:
                        std = day_data['Net PnL'].std()
                        if std > 0:
                            k = 0.5 * std; h = 3.0 * std; c_low = 0
                            for pnl in day_data['Net PnL']:
                                c_low = max(0, c_low - pnl - k)
                            if c_low > h:
                                banned_days.append(day)

                is_metrics.append({
                    'Strategy': strat, 'Family': strategy_mapping[strat],
                    'Omega': omega, 'Net PnL IS': total_net_pnl, 'Banned Days': banned_days
                })

            is_metrics_df = pd.DataFrame(is_metrics)
            if not is_metrics_df.empty:
                is_metrics_df = is_metrics_df.sort_values(by=['Omega', 'Net PnL IS'], ascending=[False, False])
                selected_strategies = []
                family_counts = {fam: 0 for fam in set(strategy_mapping.values())}

                for idx, row in is_metrics_df.iterrows():
                ...
                for rank, strat_row in enumerate(selected_strategies):
                    ...
                    historical_allocations.append({
                        ...
                        'Strategy': strat_name, 'Family': strat_row['Family'], 'Omega IS': strat_row['Omega'],
                        'Weight': weight, 'Banned Days': str(banned)
                    })
        ...
        def format_banned_days(banned_str):
            try:
                days = eval(banned_str)
                if not days: return "Nessuno"
                return ", ".join([giorni_settimana[d] for d in days])
            except:
                return "Errore"
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=recent_daily.index, y=recent_daily.values, mode='lines', fill='tozeroy', name='Cumulative Equity', line=dict(color='#00b4d8')))
        fig.update_layout(title="Equity Cumulativa WFA 28-Days Ponderata", xaxis_title="Data", yaxis_title="Net PnL ($)", margin=dict(l=40, r=40, t=40, b=40))
        st.plotly_chart(fig, use_container_width=True)

        st.header("4. Esporta Dati")
        csv_alloc = hist_alloc_df.to_csv(index=False).encode('utf-8')
